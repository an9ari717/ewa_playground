...
export default function ManagerDashboard() {
  ...
  } = useRequests("inbox", "MANAGER", 1, 5);

  // B) Recently handled (use archive; backend usually scopes by role if provided)
  const {
    data: recentData,
  ...
  } = useRequests("archive", "MANAGER", 1, 5);
  ...
  // KPIs
  const kpi = useMemo(() => {
    const pending = inboxData?.total ?? 0; // inbox == waiting for you
    let approved = 0;
    let rejected = 0;
  ...
  return (
    <Page title="Manager Dashboard" maxWidth={1100} leftOffset={60}>
      {/* Top bar */}
      <div className="dash-topbar">
        <div className="dash-topbar__left">Welcome! • {today}</div>
        <div className="dash-topbar__right">
          <Button size="sm" variant="primary" onClick={() => nav("/app/manager/inbox")}>
            Go to Inbox
          </Button>
        </div>
      </div>
      ...
      {/* Awaiting your review (Inbox preview) */}
      <Card title="Awaiting Your Review" stickyHeader stickyTop={0}>
        <div className="dash-cardbar">
          ...
          <div className="dash-cardbar__actions">
            ...
            <Button size="sm" variant="ghost" onClick={() => nav("/app/manager/inbox")}>
              View inbox
            </Button>
          </div>
        </div>
